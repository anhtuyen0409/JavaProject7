...
public class BoPhanQuanLyKhoUI extends JFrame{
	...
	protected void xuLyXoaPhieunhap() {
		...
		if(rowSelected == -1){
			...
			int ret = JOptionPane.showConfirmDialog(null, 
					"Bạn có chắc chắn xoá phiếu nhập ngày "+dcNgayNhap.getDateFormatString()+" không?",
					"Xác nhận xoá",JOptionPane.YES_NO_OPTION);
			if(ret == JOptionPane.NO_OPTION){
				return;
			}
	...
	protected void xuLySuaNhaCungCap() {
		...
		if(rowSelected == -1) {
			...
			if(txtTenNCC.getText().equals("")){
				JOptionPane.showMessageDialog(null, "Vui lòng nhập tên nhà cung cấp mới!");
			}
			else{
				if(txtDiaChi.getText().equals("")){
					JOptionPane.showMessageDialog(null, "Vui lòng nhập địa chỉ mới!");
				}
				else{
					if(txtSDT.getText().equals("")){
						JOptionPane.showMessageDialog(null, "Vui lòng nhập số điện thoại mới!");
					}
					else{
						if(txtEmail.getText().equals("")){
							JOptionPane.showMessageDialog(null, "Vui lòng nhập email mới!");
						}
						else{
							if(txtFax.getText().equals("")){
								JOptionPane.showMessageDialog(null, "Vui lòng nhập số fax mới!");
							}
							else{
								int nccSelected = Integer.parseInt(tblNhaCungCap.getValueAt(rowSelected, 0)+"");
								try {
	...
	protected void xuLySuaNhaSanXuat() {
		...
		if(rowSelected == -1) {
			...
			if(txtTenNhaSanXuat.getText().equals("")){
				JOptionPane.showMessageDialog(null, "Vui lòng nhập tên nhà sản xuất mới!");
			}
			else{
				if(txtThongTinNSX.getText().equals("")){
					JOptionPane.showMessageDialog(null, "Vui lòng nhập thông tin mới!");
				}
				else{
					int nsxSelected = Integer.parseInt(tblNhaSanXuat.getValueAt(rowSelected, 0)+"");
					try {
