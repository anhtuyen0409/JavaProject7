...
public class LoaiSanPhamService extends SQLServerService{
	...
	public Vector<LoaiSanPhamModel>timLoaiSanPhamTheoTen(String tenSP)
	{
		Vector<LoaiSanPhamModel> veclsp = new Vector<LoaiSanPhamModel>();
		try
		{
			String sql="select * from LoaiSanPham where TenLoaiSP=? and DaXoa=0";
			PreparedStatement preStatement=conn.prepareStatement(sql);
			preStatement.setString(1, tenSP);
